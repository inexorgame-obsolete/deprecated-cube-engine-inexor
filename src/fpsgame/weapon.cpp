...
namespace game
{
    ...
    void explode(bool local, fpsent *owner, const vec &v, dynent *safe, int damage, int gun)
    {
        ...
        if(gun==GUN_RL) adddynlight(v, 1.15f*guns[gun].exprad, vec(2, 1.5f, 1), 900, 100, 0, guns[gun].exprad/2, vec(1, 0.75f, 0.5f));
        else if(gun==GUN_GL) adddynlight(v, 1.15f*guns[gun].exprad, vec(0.5f, 1.5f, 2), 900, 100, 0, 8, vec(0.25f, 1, 1));
