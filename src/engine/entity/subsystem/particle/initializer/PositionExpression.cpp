/*
 * PositionExpression.cpp
 *
 *  Created on: 01.03.2015
 *      Author: aschaeffer
 */

#include "PositionExpression.h"

namespace inexor {
namespace entity {
namespace particle {

PositionExpression::PositionExpression(std::string expression)
    : EntityFunction(INITIALIZER_POSITION_EXPRESSION_FUNCTION), args(8), expression(expression)
{
...
void PositionExpression::SetExpression(std::string expression)
{
...
void PositionExpression::Execute(TimeStep time_step, EntityInstance* emitter, EntityInstance* initializer, EntityInstance* particle)
{
    ...
    try {
        ...
        (*particle)[POS]->vec3Val = vec(v[0], v[1], v[2]);
