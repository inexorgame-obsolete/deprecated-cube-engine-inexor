...
struct md3 : vertmodel, vertloader<md3>
{
    ...
    bool loaddefaultparts()
    {
        ...
        mdl.model = this;
        mdl.index = 0;
        defformatstring(name1)("%s/%s/tris.md3", modelsdir, loadname);
        mdl.meshes = sharemeshes(path(name1));
        if(!mdl.meshes)
        {
            defformatstring(name2)("%s/%s/tris.md3", modelsdir, pname);    // try md3 in parent folder (vert sharing)
    ...
    bool load()
    {
        ...
        formatstring(dir)("%s/%s", modelsdir, loadname);
        defformatstring(cfgname)("%s/%s/md3.cfg", modelsdir, loadname);
