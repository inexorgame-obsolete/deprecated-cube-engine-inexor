...
struct md2 : vertmodel, vertloader<md2>
{
    ...
    { 
        ...
        defformatstring(name1)("%s/%s/tris.md2", modelsdir, loadname);
        mdl.meshes = sharemeshes(path(name1));
        if(!mdl.meshes)
        {
            defformatstring(name2)("%s/%s/tris.md2", modelsdir, pname);    // try md2 in parent folder (vert sharing)
        ...
        defformatstring(name3)("%s/%s/md2.cfg", modelsdir, loadname);
        if(!execfile(name3, false))
        {
            formatstring(name3)("%s/%s/md2.cfg", modelsdir, pname);
