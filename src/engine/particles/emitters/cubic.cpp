...
public:
	...
	inline std::list<particle_instance*> emit(particle_emitter_instance *pe_inst, int elapsedtime)
	{
		...
		for (int i = 0; i < pe_inst->batch_size; i++)
		{
			...
			p_inst->pos->o.x = pe_inst->pos->o.x + pe_inst->attributes["size_x"] * (rx - 0.5f);
			p_inst->pos->o.y = pe_inst->pos->o.y + pe_inst->attributes["size_y"] * (ry - 0.5f);
			p_inst->pos->o.z = pe_inst->pos->o.z + pe_inst->attributes["size_z"] * (rz - 0.5f);
