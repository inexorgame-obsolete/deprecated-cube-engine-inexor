...
vec CBezierCurve::calculate_de_casteljau_coordinate(int index, int nextindex, float curvepos)
{
    ...
    // return position
    return vec(calculate_de_casteljau_coordinate(index - 1, nextindex, curvepos)).mul(1 - curvepos)
        .add(vec(calculate_de_casteljau_coordinate(index - 1, nextindex + 1, curvepos)).mul(curvepos));
}
...
SPreComputedPointStructure CBezierCurve::calculate_bernstein_coordinates(float t)
{
    ...
    for(int i = 0; i < (int) m_ParameterPoints.size(); i++)
    {
