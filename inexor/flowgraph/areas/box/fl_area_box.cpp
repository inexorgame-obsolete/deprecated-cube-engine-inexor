...
namespace vscript {
    ...
    {
        spdlog::get("global")->debug() << "[3DVS-area-box] a box area can't run any code so it can't be linked as child node!";
        return false;
    }
    ...
    bool CBoxAreaNode::OnLinkAsParentNodeAttempt(CScriptNode* child)
    {
        ...
        {
            spdlog::get("global")->debug() << "[3DVS-area-box] a box can only be linked as parent of an event node!";
            return false;
        }
        return true;
    }
