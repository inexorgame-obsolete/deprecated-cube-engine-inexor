...
namespace vscript {
    ...
    {
        spdlog::get("global")->debug() << "[3DVS-area-sphere] a sphere area can't run any code so it can't be linked as child node!";
        return false;
    }
    ...
    bool CSphereAreaNode::OnLinkAsParentNodeAttempt(CScriptNode* child)
    {
        ...
        {
            spdlog::get("global")->debug() << "[3DVS-area-sphere] a sphere can only be linked as parent of an event node!";
            return false;
        }
        return true;
    }
