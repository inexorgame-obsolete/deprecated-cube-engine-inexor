...
{
    if(identflags&IDF_OVERRIDDEN) { Log.std->error("cannot override keymap {}", *code); return; }
    keym &km = keyms[*code];
    km.code = *code;
    DELETEA(km.name);
...
{
    if(identflags&IDF_OVERRIDDEN) { Log.std->error("cannot override {0} {1}", cmd, quoted(key)); return; }
    keym *km = findbind(key);
    if(!km) { Log.std->error("unknown key {}", quoted(key)); return; }
    ...
    if(isdown)
    {
        switch(code)
        {
            ...
            case SDLK_TAB:
                ...
                {
                    complete(commandbuf, sizeof(commandbuf), commandflags&CF_EXECUTE ? "/" : nullptr, SDL_GetModState()&KMOD_SHIFT);
                    if(commandpos>=0 && commandpos>=(int)strlen(commandbuf)) commandpos = -1;
                }
                break;
...
void addcomplete(char *command, int type, char *dir, char *ext)
{
    ...
    {
        Log.std->error("cannot override complete {}", command);
        return;
    }
    if(!dir[0])
    {
