...
bool noentedit()
{
    if(!editmode) { Log.edit->error("operation only allowed in edit mode"); return true; }
    return !entediting;
}
...
int findtype(char *what)
{
    ...
    Log.edit->error("unknown entity type \"{}\"", what);
    return ET_EMPTY;
}
...
bool emptymap(int scale, bool force, const char *mname, bool usecfg)    // main empty world creation routine
{
    if(!force && !editmode) 
    {
        Log.edit->error("newmap only allowed in edit mode");
        return false;
    }
...
bool enlargemap(bool force)
{
    if(!force && !editmode)
    {
        Log.edit->error("mapenlarge only allowed in edit mode");
        return false;
    }
