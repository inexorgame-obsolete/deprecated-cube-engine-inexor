...
template<class MDL> struct skelcommands : modelcommands<MDL, struct MDL::skelmesh>
{
    ...
    static void settag(char *name, char *tagname, float *tx, float *ty, float *tz, float *rx, float *ry, float *rz)
    {
        ...
        spdlog::get("global")->error() << "could not find bone " << name << " for tag " << tagname;
    }
    ...
    static void setpitch(char *name, float *pitchscale, float *pitchoffset, float *pitchmin, float *pitchmax)
    {
        ...
        if(name[0])
        {
            ...
            spdlog::get("global")->error() << "could not find bone " << name << " to pitch";
            return;
        }
    ...
    static void setpitchtarget(char *name, char *animfile, int *frameoffset, float *pitchmin, float *pitchmax)
    {
        ...
        if(!sa) { spdlog::get("global")->error() << "could not load " << MDL::formatname() << " anim file " << filename; return; }
        skeleton *skel = ((meshgroup *)mdl.meshes)->skel;
        int bone = skel ? skel->findbone(name) : -1;
        if(bone < 0)
        {
            spdlog::get("global")->error() << "could not find bone " << name << " to pitch target";
            return;
        }
    ...
    static void setpitchcorrect(char *name, char *targetname, float *scale, float *pitchmin, float *pitchmax)
    {
        ...
        if(bone < 0)
        {
            spdlog::get("global")->error() << "could not find bone " << name << " to pitch correct";
            return;
        }
        ...
        if(target < 0)
        {
            spdlog::get("global")->error() << "could not find pitch target " << targetname << " to pitch correct " << name;
            return;
        }
        pitchcorrect c;
        c.bone = bone;
    ...
    static void setanim(char *anim, char *animfile, float *speed, int *priority, int *startoffset, int *endoffset)
    {
        ...
        else
        {
            ...
            if(!sa) spdlog::get("global")->error() << "could not load " << MDL::formatname() << " anim file " << filename;
            else loopv(anims)
            {
    ...
    static void setanimpart(char *maskstr)
    {
        ...
        loopv(bonestrs)
        {
            ...
            if(bone<0) { spdlog::get("global")->error() << "could not find bone " << bonestr << " for anim part mask [" << maskstr << "]"; bonestrs.deletearrays(); return; }
    ...
    static void setadjust(char *name, float *yaw, float *pitch, float *roll, float *tx, float *ty, float *tz)
    {
        ...
        if(!name[0]) return;
        int i = mdl.meshes ? ((meshgroup *)mdl.meshes)->skel->findbone(name) : -1;
        if(i < 0) {  spdlog::get("global")->error() << "could not find bone " << name << " to adjust"; return; }
