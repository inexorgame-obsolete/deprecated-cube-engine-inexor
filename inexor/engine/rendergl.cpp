...
void gl_checkextensions()
{
    ...
    {
        hasLATC = true;
        if(dbgexts) Log.std->debug("Using GL_EXT_texture_compression_latc extension.");
    }
    if(glversion >= 300 || hasext("GL_ARB_texture_compression_rgtc") || hasext("GL_EXT_texture_compression_rgtc"))
    {
        hasRGTC = true;
        if(glversion < 300 && dbgexts) Log.std->debug("Using GL_ARB_texture_compression_rgtc extension.");
    }
...
void cef_resize(int width, int height)
{
    if (cef_app) {
        ...
        Log.ui->debug("Update Inexor User Interface Screen Size: {0}x{1}", width, height);
...
void gl_drawmainmenu()
{
    ...
    renderbackground(nullptr, nullptr, nullptr, nullptr, true, true);
    renderpostfx();
...
{
    static Texture *icon_tex = nullptr;
    if(!icon_tex)
    {
...
static Texture *cursortex = nullptr;
