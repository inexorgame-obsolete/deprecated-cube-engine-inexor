...
{
    if(!editmode) { if(msg) Log.edit->error("operation only allowed in edit mode"); return true; }
    ...
    if(!viewable && msg) Log.edit->error("selection not in view");
    return !viewable;
}
...
void delprefab(char *name)
{
    ...
    if(p)
    {
        p->cleanup();
        prefabs.remove(name);
        Log.edit->info("deleted prefab {}", name);
    }
}
COMMAND(delprefab, "s");
...
prefab *loadprefab(const char *name, bool msg = true)
{
   ...
   if(!f) { if(msg) Log.std->error("could not read prefab {}", filename); return nullptr; }
   prefabheader hdr;
   if(f->read(&hdr, sizeof(hdr)) != sizeof(prefabheader) || memcmp(hdr.magic, "OEBR", 4)) { delete f; if(msg) Log.std->error("prefab {} has malformatted header", filename); return nullptr; }
   lilswap(&hdr.version, 1);
   if(hdr.version != 0) { delete f; if(msg) Log.std->error("prefab {} uses unsupported version", filename); return nullptr; }
   streambuf<uchar> s(f);
   block3 *copy = nullptr;
   if(!unpackblock(copy, s)) { delete f; if(msg) Log.std->error("could not unpack prefab {}", filename); return nullptr; }
   delete f;
...
static void renderprefab(prefab &p, const vec &o, float yaw, float pitch, float roll, float size, const vec &color)
{
    ...
    gle::enablevertex();
    gle::enablenormal();
    prefabmesh::vertex *v = (prefabmesh::vertex *)nullptr;
...
struct texturegui : g3d_callback
{
    ...
    void gui(g3d_gui &g, bool firstpass) override
    {
        ...
        loopi(numtabs)
        {
            g.tab(!i ? "Textures" : nullptr, 0xAAFFAA);
...
{
    if(!editmode) { Log.edit->error("operation only allowed in edit mode"); return; }
