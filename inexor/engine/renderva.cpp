...
#include "inexor/engine/grass.hpp"                    // for cleanupgrass
#include "inexor/engine/lightmap.hpp"                 // for LightMapTexture
#include "inexor/engine/octaedit.hpp"                 // for editmode
#include "inexor/engine/octarender.hpp"               // for clearvas, varoot
#include "inexor/engine/octree.hpp"                   // for vtxarray, octae...
#include "inexor/engine/pvs.hpp"                      // for pvsoccluded
#include "inexor/engine/rendergl.hpp"                 // for xtravertsva
#include "inexor/engine/renderva.hpp"                 // for endquery, start...
#include "inexor/engine/shader.hpp"                   // for Shader, lookups...
#include "inexor/engine/shadowmap.hpp"                // for isshadowmaprece...
#include "inexor/engine/water.hpp"                    // for reflecting, ref...
#include "inexor/engine/world.hpp"                    // for vertex, worldscale
#include "inexor/fpsgame/entities.hpp"                // for getents, animat...
...
void rendermapmodel(extentity &e)
{
    ...
    if(mmi) rendermodel(&e.light, mmi->name, anim, e.o, e.attr1, 0, MDL_CULL_VFC | MDL_CULL_DIST | MDL_DYNLIGHT, nullptr, nullptr, basetime);
}
...
static void reportinvalidtextslot(Slot* slot)
{
    ...
    {
        Log.std->error("Invalid texture definition (Invalid SHADER_NORMALSLMS) on index {}", slot->index);
        loopv(slot->sts)
        {
            Slot::Tex &t = slot->sts[i];
            if(t.type==TEX_DIFFUSE) Log.std->error("TEX_DIFFUSE: {}", t.name); break;
        }
