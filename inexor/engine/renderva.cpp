...
static void reportinvalidtextslot(Slot* slot)
{
    ...
    {
        Log.default->error("Invalid texture definition (Invalid SHADER_NORMALSLMS) on index {}", slot->index);
        loopv(slot->sts)
        {
            Slot::Tex &t = slot->sts[i];
            if(t.type==TEX_DIFFUSE) Log.default->error("TEX_DIFFUSE: {}", t.name); break;
        }
