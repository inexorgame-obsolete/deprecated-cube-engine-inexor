...
#define bombteamname(s) (!strcmp(s, "good") ? 1 : (!strcmp(s, "evil") ? 2 : 0))
    ...
    bool canspawn(clientinfo *ci, bool connecting)
    {
        ...
        if(gamerunning()) { Log.default->info("game is running"); return false; }
        if(notgotspawnlocations) { Log.default->info("not got spawn locations yet"); return false; }
        int i = 0;
        for(; i < spawnlocs.length(); i++) if(spawnlocs[i]->cn == ci->clientnum) break;
        if(i == spawnlocs.length()) { Log.default->info("player has got no spawn location"); return false; }
        if(ci->state.deaths==0) { Log.default->info("player has no deaths"); return true; } // ci->state.aitype!=AI_NONE &&
