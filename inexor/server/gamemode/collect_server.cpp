...
namespace server {
    ...
    collectmode_common::token &collectservermode::droptoken(const vec &o, int yaw, int team, int droptime, int dropper) {
    ...
    void collectservermode::droptokens(clientinfo *ci, bool penalty) {
    ...
    void collectservermode::leavegame(clientinfo *ci, bool disconnecting) {
        ci->state.tokens = 0;
        if(disconnecting)
        {
    ...
    void collectservermode::deposittokens(clientinfo *ci, int basenum) {
    ...
    void collectservermode::taketoken(clientinfo *ci, int id) {
    ...
    void collectservermode::initclient(clientinfo *ci, packetbuf &p, bool connecting) {
    ...
    void collectservermode::parsebases(ucharbuf &p, bool commit) {
    ...
    bool collectservermode::parse_network_message(int type, clientinfo *ci, clientinfo *cq, packetbuf &p) {
        switch(type)
        {
            case N_INITTOKENS:
