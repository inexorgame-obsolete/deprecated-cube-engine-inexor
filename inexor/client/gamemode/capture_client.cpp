...
void captureclientmode::updatebase(int i, const char *owner, const char *enemy, int converted, int ammo)
{
    ...
    if(owner[0])
    {
        ...
        {
            if(!b.name[0]) 
                spdlog::get("gameplay")->info("{0} captured base {1}", teamcolor(owner, owner), i+1);
            else if(basenumbers)
                spdlog::get("gameplay")->info("{0} captured {1}({2})", teamcolor(owner, owner), b.name, (i+1));
            else 
                spdlog::get("gameplay")->info("{0} captured {1}", teamcolor(owner, owner), b.name);
    ...
    {
        if(!b.name[0]) 
            spdlog::get("gameplay")->info("{0} lost base {1}", teamcolor(b.owner, b.owner), i+1);
        else if(basenumbers) 
            spdlog::get("gameplay")->info("{0} lost {1}({2})", teamcolor(b.owner, b.owner), b.name, (i+1));
        else 
            spdlog::get("gameplay")->info("{0} lost {1}", teamcolor(b.owner, b.owner), b.name);
        if(!strcmp(b.owner, player1->team)) 
            playsound(S_V_BASELOST);
    }
    if(strcmp(b.owner, owner)) 
        particle_splash(PART_SPARK, 200, 250, b.ammopos, owner[0] ? (strcmp(owner, player1->team) ? 0x802020 : 0x2020FF) : 0x208020, 0.24f);
    ...
    if(ammo>b.ammo)
    {
        ...
        if(icon >= 0) 
            particle_icon(vec(b.ammopos.x, b.ammopos.y, b.ammopos.z + AMMOHEIGHT + 1.0f), icon % 4, icon / 4, PART_HUD_ICON, 2000, 0xFFFFFF, 2.0f, -8);
    }
    b.ammo = ammo;
}
...
void captureclientmode::rendergame()
{
    ...
    loopv(bases)
    {
        ...
        if(b.owner[0])
        {
            ...
            if(b.enemy[0]) 
            {
                mtype = PART_METER_VS; mcolor = 0xFF1932; 
                mcolor2 = 0x3219FF; 
                if(!isowner) swap(mcolor, mcolor2); 
            }
            if(!b.name[0]) 
                formatstring(b.info, "base %d: %s", i+1, b.owner);
            else if(basenumbers) 
                formatstring(b.info, "%s (%d): %s", b.name, i+1, b.owner);
            else 
        ...
        {
            if(!b.name[0]) 
                formatstring(b.info, "base %d: %s", i+1, b.enemy);
            else if(basenumbers) 
                formatstring(b.info, "%s (%d): %s", b.name, i+1, b.enemy);
            else 
            ...
            formatstring(b.info, "base %d", i + 1);
        else if(basenumbers) 
            formatstring(b.info, "%s (%d)", b.name, i+1);
        else 
