...
namespace game {
    ...
    bool ctfclientmode::addflag(int i, const vec &o, int team, int vistime) {
    ...
    void ctfclientmode::ownflag(int i, fpsent *owner, int owntime) {
        flag &f = flags[i];
    ...
    void ctfclientmode::returnflag(int i, int vistime) {
        flag &f = flags[i];
        f.droptime = 0;
        loopv(players) players[i]->flagpickup &= ~(1<<f.id);
        f.vistime = vistime;
    ...
    void ctfclientmode::drawblip(fpsent *d, float x, float y, float s, int i, bool flagblip) {
        flag &f = flags[i];
        int blip = TEAM_NONE;
    ...
    void ctfclientmode::drawblip(fpsent *d, float x, float y, float s, const vec &pos, bool flagblip) {
    ...
    void ctfclientmode::drawhud(fpsent *d, int w, int h) {
        if(d->state == CS_ALIVE)
        {
    ...
    void ctfclientmode::removeplayer(fpsent *d) {
        loopv(flags) if(flags[i].owner_id == d->clientnum)
            {
                flag &f = flags[i];
                f.interploc.x = -1;
    ...
    vec ctfclientmode::interpflagpos(flag &f, float &angle) {
    ...
    void ctfclientmode::senditems(packetbuf &p) {
    ...
    void ctfclientmode::parseflags(ucharbuf &p, bool commit) {
        loopk(2)
        {
            int score = getint(p);
    ...
    void ctfclientmode::trydropflag() {
        if(!m_ctf) return;
    ...
    const char *ctfclientmode::teamcolorflag(flag &f) {
        return m_hold ? "the flag" : teamcolor("your flag", ctfflagteam(f.team), "the enemy flag");
    }

    void ctfclientmode::dropflag(int i, const vec &o, float yaw, int droptime) {
        flag &f = flags[i];
        f.droploc = o;
        f.droptime = droptime;
        loopv(players) players[i]->flagpickup &= ~(1<<f.id);
    ...
    void ctfclientmode::dropflag(fpsent *d, int i, int version, const vec &droploc) {
    ...
    void ctfclientmode::flagexplosion(int i, int team, const vec &loc) {
        int fcolor;
        vec color;
    ...
    void ctfclientmode::flageffect(int i, int team, const vec &from, const vec &to) {
    ...
    void ctfclientmode::returnflag(fpsent *d, int i, int version) {
        if(!flags.inrange(i)) return;
        flag &f = flags[i];
        f.version = version;
    ...
    void ctfclientmode::spawnflag(flag &f) {
    ...
    void ctfclientmode::resetflag(int i, int version, int spawnindex, int team, int score) {
        setscore(team, score);
        if(!flags.inrange(i)) return;
        flag &f = flags[i];
        f.version = version;
    ...
    void
    ctfclientmode::scoreflag(fpsent *d, int relay, int relayversion, int goal, int goalversion, int goalspawn, int team,
                             int score, int dflags, int time) {
        setscore(team, score);
    ...
    void ctfclientmode::takeflag(fpsent *d, int i, int version) {
        if(!flags.inrange(i)) return;
        flag &f = flags[i];
        f.version = version;
        f.interploc = interpflagpos(f, f.interpangle);
    ...
    void ctfclientmode::invisflag(int i, int invis) {
        if(!flags.inrange(i)) return;
        flag &f = flags[i];
    ...
    void ctfclientmode::checkitems(fpsent *d) {
    ...
    void ctfclientmode::respawned(fpsent *d) {
        vec o = d->feetpos();
        d->flagpickup = 0;
        loopv(flags)
        {
    ...
    int ctfclientmode::respawnwait(fpsent *d) {
        return m_efficiency || !m_protect ? max(0, RESPAWNSECS-(lastmillis-d->lastpain)/1000) : 0;
    }

    bool ctfclientmode::pickholdspawn(fpsent *d) {
    ...
    void ctfclientmode::pickspawn(fpsent *d) {
        if(!m_hold || !pickholdspawn(d))
            findplayerspawn(d, -1, m_hold ? 0 : ctfteamflag(d->team));
    }

    bool ctfclientmode::aihomerun(fpsent *d, ai::aistate &b) {
        if(m_protect || m_hold)
        {
    ...
    bool ctfclientmode::aicheck(fpsent *d, ai::aistate &b) {
    ...
    void ctfclientmode::aifind(fpsent *d, ai::aistate &b, vector<ai::interest> &interests) {
        vec pos = d->feetpos();
        loopvj(flags)
        {
            flag &f = flags[j];
    ...
    bool ctfclientmode::aidefend(fpsent *d, ai::aistate &b) {
        loopv(flags)
        {
            flag &g = flags[i];
            if(g.owner_id == d->clientnum) return aihomerun(d, b);
    ...
    bool ctfclientmode::aipursue(fpsent *d, ai::aistate &b) {
        if(flags.inrange(b.target))
        {
            flag &f = flags[b.target];
    ...
    bool ctfclientmode::parse_network_message(int type, ucharbuf &p) {
        switch(type)
        {
            case N_INITFLAGS:
            {
