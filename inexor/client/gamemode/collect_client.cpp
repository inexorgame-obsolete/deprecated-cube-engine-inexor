...
namespace game {
    ...
    collectmode_common::token &collectclientmode::droptoken(int id, const vec &o, int team, int droptime) {
        token &t = collectmode_common::droptoken(o, team, droptime);
        t.id = id;
        return t;
    }

    void collectclientmode::preload() {
    ...
    void collectclientmode::drawblip(fpsent *d, float x, float y, float s, const vec &pos, float size) {
    ...
    void collectclientmode::drawbaseblip(fpsent *d, float x, float y, float s, int i) {
        base &b = bases[i];
        setbliptex(b.team==collectteambase(player1->team) ? TEAM_OWN : TEAM_OPPONENT);
        drawblip(d, x, y, s, b.o);
    }

    void collectclientmode::drawhud(fpsent *d, int w, int h) {
    ...
    void collectclientmode::rendergame() {
    ...
    void collectclientmode::senditems(packetbuf &p) {
    ...
    vec collectclientmode::movetoken(const vec &o, int yaw) {
    ...
    void collectclientmode::parsetokens(ucharbuf &p, bool commit) {
        loopk(2)
        {
            int score = getint(p);
    ...
    void collectclientmode::baseexplosion(int i, int team, const vec &loc) {
        int fcolor;
        vec color;
    ...
    void collectclientmode::baseeffect(int i, int team, const vec &from, const vec &to, bool showfrom, bool showto) {
    ...
    void collectclientmode::expiretoken(int id) {
    ...
    void collectclientmode::taketoken(fpsent *d, int id, int total) {
    ...
    collectclientmode::stealtoken(fpsent *d, int id, const vec &o, int team, int yaw, int n, int basenum, int enemyteam,
        ...
        if(bases.inrange(basenum))
        {
            ...
            if(!n)
            {
                ...
                Log.game->info("{0} stole a skull from {1}", teamcolorname(d), teamcolor("your team", collectbaseteam(enemyteam), "the enemy team"));
    ...
    void collectclientmode::deposittokens(fpsent *d, int basenum, int deposited, int team, int score, int flags) {
        if(bases.inrange(basenum))
        {
            base &b = bases[basenum];
            b.laststeal = lastmillis;
        ...
        Log.game->info("{0} collected {1} {2} for {3}", teamcolorname(d), deposited, (deposited==1 ? "skull" : "skulls"), teamcolor("your team", collectbaseteam(team), "the enemy team"));
        inexor::sound::playsound(team==collectteambase(player1->team) ? S_FLAGSCORE : S_FLAGFAIL);

        if(score >= SCORELIMIT) Log.game->info("{0} collected {1} skulls", teamcolor("your team", collectbaseteam(team), "the enemy team"), score);
    }

    void collectclientmode::checkitems(fpsent *d) {
    ...
    bool collectclientmode::aicheck(fpsent *d, ai::aistate &b) {
    ...
    void collectclientmode::aifind(fpsent *d, ai::aistate &b, vector<ai::interest> &interests) {
        vec pos = d->feetpos();
        int team = collectteambase(d->team);
        if(d->tokens > 0)
        {
    ...
    bool collectclientmode::aipursue(fpsent *d, ai::aistate &b) {
        if(b.target < 0)
        {
    ...
    bool collectclientmode::parse_network_message(int type, ucharbuf &p) {
        switch(type)
        {
            case N_INITTOKENS:
