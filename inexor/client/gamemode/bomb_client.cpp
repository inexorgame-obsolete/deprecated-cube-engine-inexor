...
void bombclientmode::renderscoreboard(g3d_gui &g, scoregroup &sg, int fgcolor, int bgcolor)
{
    ...
    void bombclientmode::drawicon(int icon, float x, float y, float sz) //todo merge with other items
    {
    ...
    void bombclientmode::drawblip(fpsent *d, float x, float y, float s, const vec &pos, float size_factor) {
    ...
    void bombclientmode::renderplayersposindicator() {
        loopv(players)
        {
            fpsent *p = players[i];
    ...
    void bombclientmode::killed(fpsent *d, fpsent *actor) {
    ...
    void bombclientmode::pickspawn(fpsent *d) {
    ...
    bool bombclientmode::parse_network_message(int type, ucharbuf &p) {
        switch(type)
        {
            case N_SPAWNLOC:
            {
