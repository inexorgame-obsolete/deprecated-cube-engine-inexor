...
Texture *cubemaploadwildcard(Texture *t, const char *name, bool mipit, bool msg, bool transient)
{
    ...
    loopi(6)
    {
        ...
        if(s.w != s.h)
        {
            if(msg) Log.default->error("cubemap texture {} does not have square size", sname);
            return NULL;
        }
        ...
        {
            if(msg) Log.default->error("cubemap texture {} doesn't match other sides' format", sname);
            return NULL;
        }
...
Texture *cubemapload(const char *name, bool mipit, bool msg, bool transient)
{
    ...
    if(!strchr(pname, '*'))
    {
        ...
        if(!t)
        {
            ...
            if(!t && msg) Log.default->error("could not load envmap {}", name);
        }
    }
