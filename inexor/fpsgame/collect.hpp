...
#endif
{
    ...
    void stealtoken(fpsent *d, int id, const vec &o, int team, int yaw, int n, int basenum, int enemyteam, int score)
    {
        ...
        if(bases.inrange(basenum))
        {
            ...
            if(!n)
            {
                ...
                CLOG(INFO, "gameplay") << teamcolorname(d) << " stole a skull from " << teamcolor("your team", collectbaseteam(enemyteam), "the enemy team");
    ...
    void deposittokens(fpsent *d, int basenum, int deposited, int team, int score, int flags)
    {
        ...
        CLOG(INFO, "gameplay") << teamcolorname(d) << " collected " << deposited << " " << (deposited==1 ? "skull" : "skulls") <<" for " << teamcolor("your team", collectbaseteam(team), "the enemy team");
        playsound(team==collectteambase(player1->team) ? S_FLAGSCORE : S_FLAGFAIL);

        if(score >= SCORELIMIT) CLOG(INFO, "gameplay") << teamcolor("your team", collectbaseteam(team), "the enemy team") << " collected " << score << " skulls";
    }
