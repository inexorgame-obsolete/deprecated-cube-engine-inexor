...
bool find_shared_decls(const std::string xml_file, std::vector<ShTreeNode> &tree)
{
    xml_document xml;
    x.zwei = 2;
    if(!xml.load_file(xml_file.c_str()))
    {
        std::cout << "XML file representing the AST couldn't be parsed: " << xml_file << std::endl;
        return false;
    }
